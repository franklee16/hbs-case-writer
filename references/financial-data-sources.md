# Financial Data Sources

Sources for gathering data to build HBS cases.

## Market Data

### Stock Prices & Volumes
- **Yahoo Finance** (finance.yahoo.com) — Free, historical prices, dividends, splits
- **Google Finance** — Basic price data, news
- **FRED** (fred.stlouisfed.org) — Economic data, market indices, rates
- **TradingView** — Charts, technical indicators

### Bonds & Rates
- **FRED** — Treasury yields, corporate bond spreads, Fed funds rate
- **Treasury.gov** — Official Treasury data
- **Bloomberg** — Comprehensive fixed income data (subscription)

### Commodities
- **FRED** — Commodity price indices
- **World Bank Commodity Markets** — Historical commodity prices
- **CME Group** — Futures prices, volume

### Currencies
- **FRED** — Exchange rates, trade-weighted indices
- **OANDA** — Historical FX rates

## Company Data

### SEC Filings
- **EDGAR** (sec.gov/edgar) — All public company filings
  - 10-K (annual reports)
  - 10-Q (quarterly reports)
  - 8-K (current events)
  - DEF 14A (proxy statements)
  - Form 4 (insider trading)

### Financial Statements
- **SEC EDGAR** — Official source
- **Yahoo Finance** — Summarized financials
- **Morningstar** — Financial ratios, valuation metrics
- **Seeking Alpha** — Analysis, transcripts

### Earnings Calls
- **Seeking Alpha** — Transcripts
- **Motley Fool** — Transcripts
- **Company IR pages** — Audio and transcripts

## News & Analysis

### Financial News
- **Wall Street Journal** — Breaking news, analysis
- **Financial Times** — Global markets, deals
- **Bloomberg** — Markets, company news
- **CNBC** — Breaking market news
- **Reuters** — Wire service coverage

### Analysis & Opinion
- **Seeking Alpha** — Crowdsourced analysis
- **Morningstar** — Equity research
- **TipRanks** — Analyst ratings aggregation

## Academic & Research

### Working Papers
- **SSRN** (ssrn.com) — Working papers in finance
- **NBER** (nber.org) — Economic research
- **arXiv** — Quantitative finance papers

### Published Research
- **Google Scholar** — Academic papers
- **JSTOR** — Historical journals
- **ScienceDirect** — Elsevier journals

## Specialized Data

### Hedge Funds
- **Hedge Fund Research (HFR)** — Index performance
- **Preqin** — Fund data (subscription)
- **SEC Form ADV** — Registered advisor filings

### Private Equity
- **PitchBook** — Deal data (subscription)
- **Preqin** — Fund performance
- **Cambridge Associates** — Benchmark data

### ESG
- **MSCI ESG** — ESG ratings
- **Sustainalytics** — ESG risk ratings
- **CDP** — Climate disclosures

## Regulatory & Central Bank

### Federal Reserve
- **FRED** — Economic data
- **Federal Reserve Board** — Policy statements, Beige Book

### SEC
- **EDGAR** — Company filings
- **SEC News** — Enforcement actions, rule changes

### Other Regulators
- **FINRA** — Broker-dealer data, arbitration
- **CFTC** — Derivatives, commodity markets

## Data Collection Workflow

1. **Start with news** — Understand the narrative
   - Search WSJ, Bloomberg, FT for event coverage
   - Identify key dates, players, decisions

2. **Anchor on the primary filing** — Pull from EDGAR before relying on any secondary summary
   - S-1 / prospectus for an IPO or debt deal; 10-K / 20-F for an operating company; DEF 14A for governance and pay; 8-K for the triggering event; merger proxy for M&A
   - Enumerate segments, use of proceeds, TAM, risk factors, and key-person pay from the filing itself — run the SKILL.md [Primary-Document Coverage](../SKILL.md#primary-document-coverage) enumeration
   - Note what the issuer does *not* name (often the sharpest case angle)

3. **Gather market data** — Build exhibits
   - Yahoo Finance for price/volume data
   - FRED for economic context
   - SEC EDGAR for company-specific data

4. **Deepen with filings** — Get the details
   - 10-K for business description, risk factors
   - Earnings calls for management commentary
   - 8-K for material events

5. **Add academic context** — Ground in research
   - SSRN/NBER for relevant papers
   - For deal/valuation cases, search SSRN for a recent rigorous valuation or analysis of the same transaction; treat it as a source to engage with **and as a coverage checklist** (what segments/frameworks does it surface that your case must at least acknowledge?)
   - Cite when discussing theory or evidence

6. **Document everything** — Source all exhibits
   - Date of access
   - URL or publication reference
   - Specific data fields used
